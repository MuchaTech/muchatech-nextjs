...
import {
  PenLine, Eye, Trash2, Plus, ArrowLeft, Save, Sparkles,
  CheckCircle, Clock, User, Tag, BookOpen, AlertCircle,
  ToggleLeft, ToggleRight, Loader2, X, LogOut,
} from 'lucide-react'
...
function mdToHtml(md: string): string {
  return md
    .replace(/^## (.+)$/gm,  '<h2 class="text-xl font-bold text-[var(--tx-0)] mt-6 mb-2">$1</h2>')
    .replace(/^### (.+)$/gm, '<h3 class="text-lg font-semibold text-[var(--tx-0)] mt-4 mb-2">$1</h3>')
    .replace(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-[var(--tx-0)]">$1</strong>')
    .replace(/`(.+?)`/g, '<code class="bg-[#2BE9F0]/10 text-[#2BE9F0] px-1.5 py-0.5 rounded font-mono text-sm">$1</code>')
    .replace(/^- (.+)$/gm, '<li class="ml-4 list-disc text-[var(--tx-1)] mb-1">$1</li>')
    .replace(/\n\n/g, '</p><p class="text-[var(--tx-1)] mb-3 leading-relaxed">')
}
...
export default function BlogAdmin() {
  ...
  const deletePost = useCallback((id: string) => {
    ...
    setPosts(updated)
    savePosts(updated)
    if (active?.id === id) { setView('list'); setActive(null) }
  ...
  const togglePublish = useCallback((id: string) => {
    ...
    setPosts(updated)
    savePosts(updated)
    if (active?.id === id) setActive(a => a ? { ...a, published: !a.published } : null)
  }, [posts, active])
  ...
  /* ── AI assist ─────────────────────────────────────────────────── */
  const aiAssist = useCallback(async (task: 'excerpt' | 'content' | 'title' | 'improve') => {
    if (!active) return
    setAiLoading(true)
    setAiField(task)
    ...
    try {
      ...
      setActive(prev => {
        if (!prev) return prev
        const field    = task === 'title' ? 'title' : task === 'excerpt' ? 'excerpt' : 'content'
        const readTime = (task === 'content' || task === 'improve')
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-0)]">
      ...
      <header className="bg-[var(--bg-0)]/95 border-b border-[var(--border)] sticky top-0 z-40 nav-blur">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="/"><Logo height={34} variant={isDark ? 'dark' : 'light'} /></a>
            <span className="text-[var(--border)]">/</span>
            <span className="font-mono text-sm text-[var(--tx-2)]">Blog Admin</span>
          </div>
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <a href="/" className="hidden sm:flex items-center gap-1.5 font-mono text-xs text-[var(--tx-2)] hover:text-[#2BE9F0] transition-colors">
            ...
            <button
              ...
              onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.color = '#FC21D1'; el.style.borderColor = 'rgba(252,33,209,0.35)' }}
              onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = 'var(--tx-2)'; el.style.borderColor = 'var(--border)' }}
            >
      ...
      {/* ── Toast ───────────────────────────────────────────────── */}
      {notification && (
        <div className={`fixed top-20 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl border font-mono text-sm
          ...
          {notification.msg}
          <button onClick={() => setNotification(null)} className="ml-1 opacity-50 hover:opacity-100 transition-opacity">
        ...
        {view === 'list' && (
          <div>
            <div className="flex items-center justify-between mb-10">
              <div>
                <h1 className="font-display text-3xl font-bold text-[var(--tx-0)]">Blog Posts</h1>
            ...
            {posts.length === 0 ? (
              <div className="text-center py-24 border border-[var(--border)] rounded-2xl text-[var(--tx-2)]">
                ...
                <p className="font-mono text-sm">No posts yet. Create your first one!</p>
              </div>
            ) : (
                ...
                {posts.map(post => (
                  <div key={post.id}
                    ...
                    <div className="w-1 h-12 rounded-full flex-shrink-0"
                      style={{ background: `linear-gradient(180deg, ${post.accent}, transparent)` }} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-xs tracking-widest uppercase" style={{ color: post.accent }}>
                          {post.tag || 'No tag'}
                        </span>
                        <span className="text-[var(--border)]">·</span>
                        <span className="font-mono text-xs text-[var(--tx-3)]">{post.date}</span>
                        <span className="text-[var(--border)]">·</span>
                        <span className="font-mono text-xs text-[var(--tx-3)]">{post.readTime}m</span>
                      </div>
                      <h3 className="font-display font-semibold text-[var(--tx-0)] truncate">{post.title || 'Untitled post'}</h3>
                      <p className="text-sm text-[var(--tx-3)] truncate mt-0.5">{post.excerpt || 'No excerpt yet…'}</p>
                    </div>
                    <button onClick={() => togglePublish(post.id)}
                      ...
                      style={post.published
                        ? { borderColor: '#2BE9F040', color: '#2BE9F0', backgroundColor: '#2BE9F00D' }
                        : { borderColor: 'var(--border)',   color: 'var(--tx-2)', backgroundColor: 'var(--bg-1)' }}>
                      {post.published ? <ToggleRight className="w-3.5 h-3.5" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                      {post.published ? 'Published' : 'Draft'}
                    </button>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setActive({ ...post }); setView('preview') }}
                        className="p-2 rounded-lg text-[var(--tx-2)] hover:text-[#2BE9F0] hover:bg-[#2BE9F0]/08 transition-all" title="Preview">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => openEdit(post)}
                        className="p-2 rounded-lg text-[var(--tx-2)] hover:text-[var(--tx-0)] hover:bg-[var(--border)] transition-all" title="Edit">
                        <PenLine className="w-4 h-4" />
                      </button>
                      <button onClick={() => { if (confirm('Delete this post?')) deletePost(post.id) }}
                        className="p-2 rounded-lg text-[var(--tx-2)] hover:text-[#FC21D1] hover:bg-[#FC21D1]/08 transition-all" title="Delete">
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  style={active.published
                    ? { borderColor: '#2BE9F040', color: '#2BE9F0', backgroundColor: '#2BE9F00D' }
                    : { borderColor: 'var(--border)',   color: 'var(--tx-2)', backgroundColor: 'var(--bg-1)' }}>
                  {active.published ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-5">
                ...
                <div className="bg-[var(--bg-1)] border border-[var(--border)] rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-3">
                    <label className="font-mono text-xs text-[var(--tx-3)] uppercase tracking-widest">Title</label>
                    <button onClick={() => aiAssist('title')} disabled={aiLoading}
                      ...
                      {aiLoading && aiField === 'title' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                      AI title
                    </button>
                  </div>
                  <input
                ...
                <div className="bg-[var(--bg-1)] border border-[var(--border)] rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-3">
                    <label className="font-mono text-xs text-[var(--tx-3)] uppercase tracking-widest">Excerpt / Summary</label>
                    <button onClick={() => aiAssist('excerpt')} disabled={aiLoading || !active.title}
                      className="flex items-center gap-1 px-2.5 py-1 font-mono text-xs bg-[var(--bg-0)] border border-[#2BE9F0]/20 text-[#2BE9F0] rounded-lg hover:bg-[#2BE9F0]/08 disabled:opacity-40 transition-all">
                      {aiLoading && aiField === 'excerpt' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                      AI excerpt
                    </button>
                  </div>
                  <textarea
                ...
                <div className="bg-[var(--bg-1)] border border-[var(--border)] rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <label className="font-mono text-xs text-[var(--tx-3)] uppercase tracking-widest">Content (Markdown)</label>
                    <div className="flex gap-2">
                      <button onClick={() => aiAssist('content')} disabled={aiLoading || !active.title}
                        className="flex items-center gap-1 px-2.5 py-1 font-mono text-xs bg-[var(--bg-0)] border border-[#2BE9F0]/20 text-[#2BE9F0] rounded-lg hover:bg-[#2BE9F0]/08 disabled:opacity-40 transition-all">
                        {aiLoading && aiField === 'content' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                        AI draft
                      </button>
                      <button onClick={() => aiAssist('improve')} disabled={aiLoading || !active.content}
                        ...
                        {aiLoading && aiField === 'improve' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                        AI improve
                      </button>
                    </div>
                  </div>
                  {aiLoading && (aiField === 'content' || aiField === 'improve') ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-3">
                      <Loader2 className="w-8 h-8 animate-spin text-[#2BE9F0]" />
                      <span className="font-mono text-sm text-[var(--tx-2)]">Claude is writing your post…</span>
                    </div>
                  ) : (
                    <textarea
                      ...
                      onChange={e => {
                        ...
                        setActive(a => a
                          ? { ...a, content: text, readTime: Math.max(1, Math.ceil(text.split(' ').length / 200)) }
                          : null
                        )
                      }}
              ...
              <div className="space-y-5">
                ...
                <div className="bg-[var(--bg-1)] border border-[var(--border)] rounded-2xl p-5 space-y-4">
                  <h3 className="font-display font-semibold text-[var(--tx-0)] text-sm">Post Details</h3>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[{ col: '#2BE9F0', label: 'Cyan' }, { col: '#FC21D1', label: 'Magenta' }].map(({ col, label }) => (
                        <button key={col} onClick={() => updateField('accent', col)}
                          className="flex-1 py-2 rounded-xl border-2 font-mono text-xs font-semibold transition-all"
                          style={{
                            backgroundColor: `${col}14`,
                            color: col,
                            borderColor: active.accent === col ? col : 'transparent',
                  ...
                  <div className="pt-2 border-t border-[var(--border)] space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs text-[var(--tx-3)]">Est. read time</span>
                      <span className="font-mono text-sm font-semibold text-[#2BE9F0]">{active.readTime} min</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs text-[var(--tx-3)]">Word count</span>
                ...
                {/* AI assistant */}
                <div className="rounded-2xl p-5 border border-[#2BE9F0]/15"
                  style={{ background: 'linear-gradient(135deg,rgba(43,233,240,0.04),rgba(252,33,209,0.03))' }}>
                  <div className="flex items-center gap-2 mb-3">
                    <Sparkles className="w-4 h-4 text-[#2BE9F0]" />
                    <span className="font-display font-semibold text-[var(--tx-0)] text-sm">AI Writing Assistant</span>
                  </div>
                  <p className="text-[var(--tx-2)] text-xs leading-relaxed mb-4">
                    Claude can help draft cybersecurity content tailored for the South African market.
                  </p>
                  <div className="space-y-2">
                    {[
                      { task: 'title'   as const, label: 'Generate title from tag',  col: '#2BE9F0', disabled: false },
                      { task: 'excerpt' as const, label: 'Write excerpt from title', col: '#2BE9F0', disabled: !active.title },
                      { task: 'content' as const, label: 'Draft full post with AI',  col: '#2BE9F0', disabled: !active.title },
                      { task: 'improve' as const, label: 'Improve existing content', col: '#FC21D1', disabled: !active.content },
                    ].map(({ task, label, col, disabled }) => (
                      <button key={task} onClick={() => aiAssist(task)} disabled={aiLoading || disabled}
                        ...
                        {aiLoading && aiField === task
                          ? <Loader2 className="w-3.5 h-3.5 animate-spin flex-shrink-0" style={{ color: col }} />
                          : <Sparkles className="w-3.5 h-3.5 flex-shrink-0" style={{ color: col }} />}
                        {label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="bg-[var(--bg-1)] border border-[#FC21D1]/20 rounded-2xl p-5">
                  <h4 className="font-mono text-xs text-[#FC21D1] uppercase tracking-widest mb-3">Danger Zone</h4>
                  <button
                    onClick={() => { if (confirm('Delete this post? This cannot be undone.')) { deletePost(active.id); setView('list') } }}
          ...
          <div>
            <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
              ...
              <span className="font-mono text-xs px-3 py-1.5 rounded-full border"
                style={active.published
                  ? { backgroundColor: '#2BE9F00D', color: '#2BE9F0', borderColor: '#2BE9F040' }
                  : { backgroundColor: 'var(--bg-1)',   color: 'var(--tx-2)', borderColor: 'var(--border)' }}>
                {active.published ? 'Published' : 'Draft'}
              </span>
            </div>

            <div className="max-w-3xl mx-auto">
              <div className="h-1.5 rounded-t-xl mb-8"
                style={{ background: `linear-gradient(90deg, ${active.accent}, transparent)` }} />
              <span className="font-mono text-xs uppercase tracking-widest mb-4 block" style={{ color: active.accent }}>
                {active.tag}
              </span>
              ...
              <div className="flex items-center gap-4 mb-6 pb-6 border-b border-[var(--border)]">
                ...
                <span className="font-mono text-xs text-[var(--tx-2)]">{active.date}</span>
              </div>
              {active.excerpt && (
                <p className="text-[var(--tx-1)] text-lg leading-relaxed mb-8 italic border-l-4 pl-4"
                  style={{ borderColor: active.accent }}>
                  {active.excerpt}
                </p>
              )}
              <div className="prose-content"
                dangerouslySetInnerHTML={{ __html: `<p class="text-[var(--tx-1)] mb-3 leading-relaxed">${mdToHtml(active.content)}</p>` }}
              />
              {!active.content && (
                <p className="text-[var(--tx-2)] font-mono text-sm italic">No content yet — go back to write your post.</p>
              )}
            </div>
          </div>
        )}
